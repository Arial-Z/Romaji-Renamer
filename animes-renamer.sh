...
do
	...
	then
		if awk -F"\t" '{print $1}' "$SCRIPT_FOLDER/data/tmp/plex_animes_export.tsv" | grep -q -w "$tvdb_id"
		then
			line=$(awk -F"\t" '{print $1}' "$SCRIPT_FOLDER/data/tmp/plex_animes_export.tsv" | grep -w -n "$tvdb_id" | cut -d : -f 1)
			plex_title=$(sed -n "${line}p" "$SCRIPT_FOLDER/data/tmp/plex_animes_export.tsv" | awk -F"\t" '{print $2}')
			asset_name=$(sed -n "${line}p" "$SCRIPT_FOLDER/data/tmp/plex_animes_export.tsv" | awk -F"\t" '{print $3}')
			seasons_list=$(sed -n "${line}p" "$SCRIPT_FOLDER/data/tmp/plex_animes_export.tsv" | awk -F"\t" '{print $4}')
...
done < "$SCRIPT_FOLDER/data/tmp/plex_animes_export.tsv"
printf "%s - Done\n\n" "$(date +%H:%M:%S)" | tee -a "$LOG"
...
:> "$SCRIPT_FOLDER/data/tmp/ongoing-tmp.tsv"
ongoingpage=1
...
do
	...
	--data '{ "query": "{ Page(page: '"$ongoingpage"', perPage: 50) { pageInfo { hasNextPage } media(type: ANIME, status_in: RELEASING, sort: POPULARITY_DESC) { id } } }" }' > "$SCRIPT_FOLDER/data/tmp/ongoing-anilist.json" -D "$SCRIPT_FOLDER/data/tmp/anilist-limit-rate.txt"
	rate_limit=0
	...
	jq '.data.Page.media[].id' -r "$SCRIPT_FOLDER/data/tmp/ongoing-anilist.json" >> "$SCRIPT_FOLDER/data/tmp/ongoing-tmp.tsv"		# store the mal ID of the ongoing show
	if grep -q -w ":false}" "$SCRIPT_FOLDER/data/tmp/ongoing-anilist.json"														# stop if page is empty
	then
		break
	fi
	((ongoingpage++))
...
sort -n "$SCRIPT_FOLDER/data/tmp/ongoing-tmp.tsv" | uniq > "$SCRIPT_FOLDER/data/tmp/ongoing.tsv"
...
done < "$SCRIPT_FOLDER/data/tmp/ongoing.tsv"
printf "%s - Done\n\n" "$(date +%H:%M:%S)"
