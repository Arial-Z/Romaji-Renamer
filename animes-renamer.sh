...
printf "%S - exporting Plex anile library\n" "$(date +%H:%M:%S)"
...
printf "%s - Creating anilist airing list" "$(date +%H:%M:%S)"
...
do
	printf "%s - Wrinting metadata for tvdb id : %s / Anilist id : %s \n" "$(date +%H:%M:%S)" "$tvdb_id" "$anilist_id"
	write-metadata
