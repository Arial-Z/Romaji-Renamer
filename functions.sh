...
function get-anilist-infos () {
	...
	then
		...
		--data '{ "query": "{ Media(type: ANIME, id: '"$anilist_id"') { title { romaji, english  }, averageScore, genres, tags { name },studios { edges { node { name, isAnimationStudio } } }, coverImage { extraLarge }, idMal} }" }' > "$SCRIPT_FOLDER/data/anilist-$anilist_id.json" -D "$SCRIPT_FOLDER/tmp/anilist-limit-rate.txt"
		rate_limit=0
