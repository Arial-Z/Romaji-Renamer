...
function download-anime-id-mapping () {
	...
	do
		...
		then
			curl -s "https://raw.githubusercontent.com/Arial-Z/Animes-ID/main/list-animes-id.json" > "$SCRIPT_FOLDER/data/tmp/list-animes-id.json"
			size=$(du -b "$SCRIPT_FOLDER/data/tmp/list-animes-id.json" | awk '{ print $1 }')
		else
			curl -s "https://raw.githubusercontent.com/Arial-Z/Animes-ID/main/list-movies-id.json" > "$SCRIPT_FOLDER/data/tmp/list-movies-id.json"
			size=$(du -b "$SCRIPT_FOLDER/data/tmp/list-movies-id.json" | awk '{ print $1 }')
		fi
			((wait_time++))
		if [[ $size -gt 1000 ]]
		then
...
function get-anilist-id () {
	if [[ $media_type == "animes" ]]
	then
		jq --arg tvdb_id "$tvdb_id" '.[] | select( .tvdb_id == $tvdb_id ) | select( .tvdb_season == "1" or .tvdb_season == "-1" ) | select( .tvdb_epoffset == "0" ) | .anilist_id' -r "$SCRIPT_FOLDER/data/tmp/list-animes-id.json" | head -n 1
	else
		jq --arg imdb_id "$imdb_id" '.[] | select( .imdb_id == $imdb_id ) | .anilist_id' -r "$SCRIPT_FOLDER/data/tmp/list-movies-id.json" | head -n 1
	fi
}
function get-mal-id () {
	invalid_mal_id=0
...
function get-tvdb-id () {
	jq --arg anilist_id "$anilist_id" '.[] | select( .anilist_id == $anilist_id ) | .tvdb_id' -r "$SCRIPT_FOLDER/data/tmp/list-animes-id.json"
}
...
function get-mal-infos () {
	...
	then
		...
		curl -s -o "$SCRIPT_FOLDER/config/data/MAL-$mal_id.json" -w "%{http_code}" "https://api.jikan.moe/v4/anime/$mal_id" > "$SCRIPT_FOLDER/data/tmp/jikan-limit-rate.txt"
		if grep -q -w "429" "$SCRIPT_FOLDER/data/tmp/jikan-limit-rate.txt"
		then
...
function download-airing-info () {
	...
	then
		...
		-X POST \
		-H 'content-type: application/json' \
		--data '{ "query": "{ Media(type: ANIME, id: '"$anilist_id"') { relations { edges { relationType node { id type format title { romaji } status } } } } }" }' > "$SCRIPT_FOLDER/config/data/relations-$anilist_id.json" -D "$SCRIPT_FOLDER/data/tmp/anilist-limit-rate.txt"
		rate_limit=0
		rate_limit=$(grep -oP '(?<=x-ratelimit-remaining: )[0-9]+' "$SCRIPT_FOLDER/data/tmp/anilist-limit-rate.txt")
		if [[ rate_limit -lt 3 ]]
		then
...
function get-airing-status () {
	...
	do
		...
		then
			...
			:> "$SCRIPT_FOLDER/data/tmp/airing_sequel_tmp.json"
			while IFS=$'\n' read -r anilist_id
			do
				download-airing-info
				cat "$SCRIPT_FOLDER/config/data/relations-$anilist_id.json" >> "$SCRIPT_FOLDER/data/tmp/airing_sequel_tmp.json"
			done < "$SCRIPT_FOLDER/data/tmp/airing_sequel_tmp.txt"
			anilist_id=$anilist_multi_id_backup
			sequel_data=$(jq '.data.Media.relations.edges[] | select ( .relationType == "SEQUEL" ) | .node | select ( .format == "TV" or .format == "ONA" or .format == "MOVIE" or .format == "OVA" )' -r "$SCRIPT_FOLDER/data/tmp/airing_sequel_tmp.json")
			if [ -z "$sequel_data" ]
			then
				airing_status="Ended"
				...
				then
					...
					if [[ $sequel_multi_check -gt 0 ]]
					then
						printf "%s" "$anilist_id" > "$SCRIPT_FOLDER/data/tmp/airing_sequel_tmp.txt"
			...
			then
				...
				then
					...
					if [[ $sequel_multi_check -gt 0 ]]
					then
						printf "%s\n" "$anilist_id" > "$SCRIPT_FOLDER/data/tmp/airing_sequel_tmp.txt"
						anilist_id=$( printf "%s" "$anilist_id" | head -n 1)
						((last_sequel_found++))
					else
						((last_sequel_found++))
...
function get-season-infos () {
	anilist_backup_id=$anilist_id
	season_check=$(jq --arg anilist_id "$anilist_id" '.[] | select( .anilist_id == $anilist_id ) | .tvdb_season' -r "$SCRIPT_FOLDER/data/tmp/list-animes-id.json")
		...
		do
			...
			then
				...
				then
					...
					anilist_id=$(jq --arg tvdb_id "$tvdb_id" --arg season_number "$season_number" '.[] | select( .tvdb_id == $tvdb_id ) | select( .tvdb_season == $season_number ) | select( .tvdb_epoffset == "0" ) | .anilist_id' -r "$SCRIPT_FOLDER/data/tmp/list-animes-id.json" | head -n 1)
					if [[ -n "$anilist_id" ]]
					then
						get-anilist-infos
