...
then
        ...
        while read -r mal_id
        do
                if awk -F"\t" '{print $2}' $SCRIPT_FOLDER/ID/movies.tsv | grep -w "\<$mal_id\>"		# create the top movies list
		then
...
do
	if grep -w "\<$mal_id\>" $movies_titles
	then
		get-mal-infos
		get-mal-poster
		...
		then
			...
			if awk -F"\t" '{print $2}' $SCRIPT_FOLDER/data/top-movies.tsv | grep -w "\<$mal_id\>"
			then
		...
		if awk -F"\t" '{print $2}' $SCRIPT_FOLDER/data/top-movies.tsv | grep -w "\<$mal_id\>"		# Movies-top-100 label
		then
